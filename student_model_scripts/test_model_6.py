...
student_model.load_state_dict(torch.load("trained_models/depth_student_mobilenetv2.pth", map_location=DEVICE))

teacher_model = DepthAnythingV2(encoder="vitb", features=128, out_channels=[96, 192, 384, 768]).to(DEVICE).eval()
teacher_model.load_state_dict(torch.load("checkpoints/depth_anything_v2_vitb.pth", map_location=DEVICE))
...
# === 컬러맵 변환 ===
teacher_colored = cv2.applyColorMap(teacher_map, cv2.COLORMAP_JET)
student_colored = cv2.applyColorMap(student_map, cv2.COLORMAP_JET)
...
cv2.imwrite("comparison_output/student_depth.png", student_map)
cv2.imwrite("comparison_output/error_map.png", error_map)
cv2.imwrite("comparison_output/depth_anything_v2_vitb_colored.png", teacher_colored)
cv2.imwrite("comparison_output/student_colored.png", student_colored)
cv2.imwrite("comparison_output/error_colored.png", error_colored)

# === 출력 ===
print(f"\n결과 요약")
print(f"depth_anything_v2_vitb 평균 추론 시간: {teacher_time:.4f}s ± {teacher_std:.4f}")
print(f"학생 모델 평균 추론 시간: {student_time:.4f}s ± {student_std:.4f}")
